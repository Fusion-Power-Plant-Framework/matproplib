...
This file was autogenerated from matml31.xsd using xsdata-pydantic.
It has been modified and cleaned up for our uses
"""
...
class BulkDetails(MatMLBase):
    ...
    description: str | None = field(
        default=None, metadata={"name": "Description"} | t_element
    )

    @model_validator(mode="after")
    def _out_of_spec(self):
        out_of_spec_template = (
            "MatML file not in spec. '{}' is not a valid key. Please use '{}'."
        )
        if self.description is not None:
            # ANSYS out of spec
            log.warning(
                out_of_spec_template.format(
                    "BulkDetails:Description", "BulkDetails:Notes"
                )
            )
            self.notes = (
                self.description if self.notes is None else self.notes + self.description
            )
...
class Material(MatMLBase):
    ...
    id: str = field(default_factory=lambda: material_id(), metadata=t_attr)  # noqa: PLW0108
